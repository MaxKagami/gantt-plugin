...
evil.block('@@resizable',
  {
    ...
    "on resiz_head": function(ev) {
      ...
      //   }
      // });
      // jQuery('.gantt.container .chart .heading').height(jQuery('.gantt.container .grid .heading').height());
      // jQuery('.gantt.container .chart .row_i_' + id).height(this.block.height());
      // console.log(jQuery('.gantt.container .chart .row_i_' + id).height());
    },
...
evil.block('@@movable2',
  {
    ...
    'pointerup on body': function(ev) {
      ...
      $('[data-block="resizable"]').trigger('resiz');
      // $('[data-block="resizable"]').each(function(idx, el) { console.log(el); el.trigger('resiz'); });
      $.ajax(this.url, {dataType: 'json', data: { name: this.col, value: this.gsize + w }, method: 'PUT', success: function(res, rep) {
      }});
      // console.log($('[data-block="resizable"] .issue .' + this.col).trigger('resiz_head'));
    }
  }
);
